...
const OUT_DIR = 'D:/Coding/AgentMaster/Tool-Stack-Reference';

try {
  const htmlPath = OUT_DIR + '/master-hub.html';
  ...
  fs.writeFileSync(OUT_DIR + '/hub-tools-out.json', JSON.stringify(toolsGrouped, null, 2));
  fs.writeFileSync(OUT_DIR + '/hub-tech-out.json', JSON.stringify(techGrouped, null, 2));
  ...
  fs.writeFileSync(OUT_DIR + '/hub-summary.json', JSON.stringify(summary, null, 2));

  process.stderr.write('Done! Categories: ' + Object.keys(toolsGrouped).length + '\n');
} catch(e) {
